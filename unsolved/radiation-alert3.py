...
def raycast(source, walls, barriers):
    ...
    queue = []
    collection = list(map(lambda x: (x,True),walls))
    collection += list(map(lambda x: (x,False),barriers))

    for (x,y),is_wall in collection:
        segment = Triangle(x,y)

        if (not is_wall):
            if (point_isclose(source, segment.p1)):
                continue

            if (point_isclose(source, segment.p2)):
                continue

            #Check if the segment is parallel to the source
            ...
            if (math.isclose(angle1, angle2) or
                math.isclose(angle2, angle3)):
                continue

        #Check if there is collision at 0/360 degrees
    ...
    while (len(queue) > 0):
        ...
        for i,triangle in enumerate(triangles[::]):
            ...
            longer = False
            if (segment_longer(segment,triangle)):
                #print("Longer")
                longer = True
...
def main():
    ...
    while (line):
        ...
        walls = []
        walls.append((territory[-1],territory[0]))
        for i in range(1,nverts):
            walls.append((territory[i-1],territory[i]))
        ...
        area_covered = raycast(radiation, walls, barriers);
